...
/** Thin wrapper so the hook re-runs when the active poem changes */
function CarouselActions({ poem }: { poem: any }) {
  const { isLiked, likes, isSaved, toggleLike, toggleSave, share, download } =
    usePostActions(poem);

  return (
    <div
      className="glass-panel flex flex-wrap items-center gap-4 rounded-[20px] px-5 py-4 text-slate-600"
    ...
    >
      ...
      >
        <Heart
          className={`h-5 w-5 ${isLiked ? 'fill-rose-500 text-rose-500' : 'text-sky-600'}`}
        />
...
export function TopPoemsCarousel({ onPostClick }: TopPoemsCarouselProps) {
  ...
  const prevSlide = () =>
    setCurrentIndex((prev) =>
      topPoems.length ? (prev - 1 + topPoems.length) % topPoems.length : 0
    );
    ...
    >
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="icon"
            onClick={prevSlide}
            className="glass-panel rounded-[20px] border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
          >
      ...
      <div className="glass-card relative overflow-hidden rounded-[20px]">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {/* Poem text — clicking opens detail page */}
            <button
              className="flex max-w-3xl flex-col gap-6 text-left"
            ...
            <div className="flex flex-col gap-4 lg:min-w-[280px] lg:items-end">
              <CarouselActions poem={currentPoem} />
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
